...
def L_U(y, gamma, alpha):
    return -torch.abs(y - gamma) * torch.log(torch.exp(alpha - 1) - 1)
...
def URERN_loss(dist_params, y, lambda_1=1.0, lambda_2=1.0):
    gamma, v, alpha, beta = dist_params
    
    # Calculate L^ERN (your existing evidential regression loss)
    L_ERN = nig_nll(gamma, v, alpha, beta, y) + lambda_1 * nig_reg(gamma, v, alpha, beta, y)
    
    # Calculate L^U
    L_U_term = L_U(y, gamma, alpha)
    
    # Combine the losses according to equation (19)
    loss = L_ERN + lambda_2 * L_U_term.mean()
